...
# ====================================================================== #
# Pure helpers — moved to engine.backtest_lib (2026-07 layout pass) so scripts/ stays
# pure CLIs; re-exported here so every existing caller/test keeps its import path.
# ====================================================================== #
from engine.backtest_lib import (  # noqa: E402,F401
    cost_bps_for_adv, drift_weights, max_drawdown, month_start_dates,
    portfolio_metrics, sleeve_return, transaction_cost, turnover,
)


def _latest_adv(latest_date) -> "pd.Series":
    """Backcompat shim → :func:`engine.backtest_lib.latest_adv` (arg was never used)."""
    from engine.backtest_lib import latest_adv
    return latest_adv(PRICES_DIR)
